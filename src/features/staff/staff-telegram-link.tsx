 ...
 * also goes to a super admin: removing a link only takes authority away. Hiding is a courtesy; the
 ...
 * moment the dialog closes. While it is open the account is re-read every few seconds, so "linked"
 * appears as soon as the bot has taken the code — nothing pushes that to the console.
 */
export function StaffTelegramLink({ admin }: { admin: AdminUser }) {
  const t = useT(staffMessages);
  const { admin: me, role, can } = useAuth();
  ...
  const mayLink = !admin.telegramLinked && (isSelf || role === 'PLATFORM_ADMIN');
  const mayUnlink = admin.telegramLinked && (isSelf || can('admins.write'));

  // Polls only while a code is out and not yet redeemed; the page's own read shares this cache.
  const live = useAdmin(admin.id, {
    refetchIntervalMs: open && issue.data !== undefined && !admin.telegramLinked ? TENANT_CHAT_POLL_MS : false,
  });
  ...
  return (
    <div className="flex flex-col items-end gap-1.5">
      ...
      <Dialog open={open} onOpenChange={close}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('staff.telegram.dialogTitle', { name: admin.displayName })}</DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-3 text-sm">
            ...
            {linkedNow && code !== undefined ? (
              <Alert tone="success" title={t('staff.telegram.linkedTitle', { name: admin.displayName })}>
            ...
            {code === undefined || linkedNow ? null : (
              <>
                ...
                <p className="text-xs text-[var(--muted-foreground)]">{t('staff.telegram.waiting')}</p>
              </>
            )}
          </div>
